...
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean; error: Error | null }> {
  ...
  render() { /* ... ErrorBoundary render logic ... */
    if (this.state.hasError) {
      return ( <div style={{ padding: '20px', margin: '20px', border: '2px dashed red', borderRadius: '8px', backgroundColor: '#fff0f0' }}> <h1 style={{ color: 'red', marginBottom: '10px' }}>Oops! Bir Hata Oluştu.</h1> <p>Uygulamanın bu bölümü görüntülenirken bir sorun yaşandı.</p> <p>Lütfen tarayıcı konsolunu kontrol edin veya daha sonra tekrar deneyin.</p> {this.state.error && ( <details style={{ marginTop: '10px', whiteSpace: 'pre-wrap', background: '#ffe0e0', padding: '5px', borderRadius: '4px' }}> <summary>Hata Detayları</summary> {this.state.error.toString()} </details> )} </div> );
    } return this.props.children;
 }
}

// --- Environment Variable Simulation ---
const IS_DEMO_MODE = false; // Hardcoded for preview. In real app, check process.env.NEXT_PUBLIC_DEMO_MODE !== 'no'
...
// --- Mock Components ---
// MockIngredientSelector now uses 'ingredientsToUse'
function MockIngredientSelector({ /* ... props ... */ selected, onSelect, onClose }: { selected: Ingredient[]; onSelect: (ingredient: Ingredient) => void; onClose: () => void; }) {
  // Get unique categories dynamically from the selected data source
  const categories = React.useMemo(() => {
      if (!Array.isArray(ingredientsToUse)) { console.error("Ingredients data is not an array!", ingredientsToUse); return []; }
      const uniqueCategories = new Set(ingredientsToUse.map(ing => ing.category));
      ...
      return Array.from(uniqueCategories).sort((a, b) => {
          const indexA = categoryOrder.indexOf(a.toLowerCase()); const indexB = categoryOrder.indexOf(b.toLowerCase());
          if (indexA === -1 && indexB === -1) return a.localeCompare(b, 'tr'); if (indexA === -1) return 1; if (indexB === -1) return -1; return indexA - indexB;
      });
  }, [ingredientsToUse]); // Depend on ingredientsToUse
  ...
  return (
    <div className="p-4 border rounded-lg bg-white shadow-md mb-4">
      ...
      <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto mb-4 pr-2">
        {/* Filter and map using ingredientsToUse */}
        {ingredientsToUse
...
// CustomRecipePage component definition (Unchanged functionally)
function CustomRecipePage({ onNavigate }: { onNavigate: (path: string) => void }) { /* ... CustomRecipePage code ... */
  const [selectedIngredients, setSelectedIngredients] = useState<Ingredient[]>([]); const [showSelector, setShowSelector] = useState(false); const [isLoading, setIsLoading] = useState(false); const [recipe, setRecipe] = useState<any>(null); const [error, setError] = useState<string | null>(null); const [currentStep, setCurrentStep] = useState(0);
