...
function LandingPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  <button
    ...
    className={`px-4 py-2 rounded-full shadow ${selectedDevice === "thermomix" ? "bg-green-600 text-white" : "bg-gray-200"}`}
  >
    Thermomix
  </button>
  <button
    onClick={() => setSelectedDevice("thermogusto")}
    className={`px-4 py-2 rounded-full shadow ${selectedDevice === "thermogusto" ? "bg-green-600 text-white" : "bg-gray-200"}`}
  >
    ThermoGusto
  </button>
</div>
